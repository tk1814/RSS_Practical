...
targetPosition -= robotConfigs['robotStartPos']

pltTime, pltEFPosition, pltDisArray = sim.move_without_PD(endEffector, targetPosition, speed=0.01, orientation=None,
...
zline = np.ravel(pltEFPosition[:, 2].reshape(1, pltEFPosition.shape[0]))
xline = np.ravel(pltEFPosition[:, 0].reshape(1, pltEFPosition.shape[0]))
yline = np.ravel(pltEFPosition[:, 1].reshape(1, pltEFPosition.shape[0]))
ax.plot3D(xline, yline, zline, 'blue')

task1_figure_name = "task1_kinematics_2.png"
task1_savefig = True
if task1_savefig:
...
plt.plot(pltTime, np.array(pltDisArray).flatten(), color='blue')  # pltEFPosition
plt.xlabel("Time s")
